...
class dbHandler(logging.Handler):
    '''
    def __init__(self):
        self.config_engine = create_engine(Config.SQLALCHEMY_DATABASE_URI,echo=False,logging_name='adenlog')
        self.ConfigSession = sessionmaker(bind=self.config_engine)
        self.config_session = scoped_session(self.ConfigSession)
        metadata = MetaData(self.config_engine)
        log_table = Table('tblLogs',metadata,
                          ...
                          Column('Site'),
                          Column('User'))
        # mapper(LogModel,log_table)
        self.LogModel = getModel(log_table, self.config_engine)
    ...
    def emit(self, record):
        dic = {'LogRecord':record,'data':request.json,'method':request.full_path,'UserIP':request.remote_addr}
        try:
            if g.get('Site'):
                dic['Site'] = g.get('Site')
            if g.get('User'):
    ...
    def emit2(self, record):
        ...
        try:
            self.config_session.add(self.log)
            self.config_session.commit()
        except:
